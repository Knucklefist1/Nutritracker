document.addEventListener("DOMContentLoaded", (e) => {
      ...
      if (view == "thirtyDays") {
        container.appendChild(
          generateMealAndDrinkTableMonth(
            mealRecords.meals,
            mealRecords.activities,
            mealRecords.ingredients,
            bmrCalculatedValue
          )
        );
      } else {
        container.appendChild(
          generateMealAndDrinkTableDay(
            mealRecords.meals,
            mealRecords.activities,
            mealRecords.ingredients,
            bmrCalculatedValue
   ...
   // Beregn dato 30 dage tilbage
    ...
    mealRecords.filter((rec) => {
      ...
      rec.drinkDate = new Date(rec.Date)
        ...
        .slice(0, 10)
        .replace("T", " ");
      rec.drinkTime = new Date(
        rec.drinkDate + " " + rec.DrinkTime
      ).toISOString();
    ...
    mealRecords.forEach((record) => {
      ...
      dayTotals[recordDate].calories += record.nutrients
        ? record.nutrients.kcal || 0
        : 0;
      dayTotals[recordDate].water += parseFloat(record.drinkVolume || 0);
    });

    ingredientsRecords.filter((rec) => {
      rec.nutrients = JSON.parse(rec.Nutrients);
      rec.drinkDate = new Date(rec.Date)
        .toISOString()
        .slice(0, 10)
    ...
    // Calculate totals for meal records
    ingredientsRecords.forEach((record) => {
      const recordDate = new Date(record.Date)
        .toISOString()
        .slice(0, 10)
        .replace("T", " ");
    ...
    activityRecords.forEach((record) => {
      ...
      dayTotals[recordDate].caloriesBurned += parseFloat(
    ...
    Object.keys(dayTotals).forEach((date) => {
      ...
      cellCaloriesBurned.textContent = (
        Number(dayTotals[date].caloriesBurned) + Number(BmrCalPerHour)
      ).toFixed(2);
    });
  ...
  function generateMealAndDrinkTableDay(
    ...
    mealRecords.forEach((record) => {
      ...
      hourlyTotals[recordHour] = hourlyTotals[recordHour] || {
        water: 0,
        calories: 0,
        caloriesburned: 0,
      };
      hourlyTotals[recordHour].calories += record.nutrients
        ? record.nutrients.kcal || 0
        : 0;
    });
    ...
    let BmrCalPerHour = bmrCalculatedValue
      ? (bmrCalculatedValue / 24).toFixed(2)
      : 0;
    for (let hour = 0; hour <= 23; hour++) {
      ...
      cellCalories.textContent = hourlyTotals[hour]
        ? hourlyTotals[hour].calories.toFixed(2)
        : 0;
      cellBurnCalories.textContent = hourlyTotals[hour]
        ? Number(hourlyTotals[hour].caloriesburned) + Number(BmrCalPerHour)
        : BmrCalPerHour;
    }
    return table;
  }
